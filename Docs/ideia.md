...
- `POST /alunos`, `GET /alunos`, `PUT /alunos/:id`, `DELETE /alunos/:id` – CRUD protegido por `authenticate` + `requireRole('dono')`, filtra por `academia_id` e força `role = 'aluno'`. `GET /alunos` e `GET /alunos/:id` também podem ser usados por `funcionario` para revisar estudantes da mesma academia. Além disso, adicionamos `GET /alunos/me` e `PUT /alunos/me` para permitir que o próprio `aluno` autenticado consulte e atualize seu registro (nome/email) dentro do mesmo `academia_id` do token e respeitando as constraints da tabela `Usuarios`. A senha permanece protegida no campo `password_hash` do schema (`sql/schema.sql`), então qualquer alteração de senha deve passar pelo fluxo dedicado de `auth` (ex.: um endpoint separado para redefinir a senha).
