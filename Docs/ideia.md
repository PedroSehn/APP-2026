...
- `POST /alunos`, `GET /alunos`, `PUT /alunos/:id`, `DELETE /alunos/:id` – CRUD protegido por `authenticate` + `requireRole('dono')`, filtra por `academia_id` e força `role = 'aluno'`. `GET /alunos` e `GET /alunos/:id` também podem ser usados por `funcionario` para revisar estudantes da mesma academia. Além disso, adicionamos `GET /alunos/me` e `PUT /alunos/me` para permitir que o próprio `aluno` autenticado consulte e atualize seu registro (nome/email/senha) dentro do mesmo `academia_id` do token, respeitando as constraints da tabela `Usuarios`.
